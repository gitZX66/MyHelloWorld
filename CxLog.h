...
public:
	...
	void SetLevel( const EnumType eType )
	{
		_eType = eType;
	}
	...
	struct Item
	{
		...
		Item() 
		{
			InitItem(NULL, NULL, 0, CX_LOG_INFO, NULL, NULL, 0);
		}
		...
		VOID AllocStringBuffer(LPTSTR& szDest, LPCTSTR& szSrc)
		{
			if(szSrc)
			{
				ULONG uSize = (ULONG)_tcslen(szSrc) + 1;
		...
		{
			static LPCTSTR szType[] = {_T("Debug") ,_T("Info"), _T("Warning"), _T("Exception"), _T("Error")};
	...
	TCHAR* GetFilePath()
	{
		...
		if(!bFlag)
		{
			...
			time_t now = time(NULL);
			tm *tmNow = localtime(&now);//获取当前系统时间用于创建文件夹名称
			CString strTime =_T("");
			strTime.Format(_T("%04d%02d%02d"),tmNow->tm_year+1900,tmNow->tm_mon+1,tmNow->tm_mday);
			...
			if(dwLen)
			{
				...
				while(p>szFilePath && *p!=_T('\\'))
					p--;
				if(*p == _T('\\'))
					_tcscpy(p, _T("\\log\\")+strTime+"\\");
			}
			else
				_tcscpy(szFilePath, _T(".\\log\\")+strTime+"\\");
...
public:
	...
	CxLog(bool &alive) : _bAlive(alive)
	{
		...
		DWORD dwSize = GetModuleFileName(NULL,szCurPath,MAX_PATH);
		if( dwSize > 0){
			PathStripPath(szCurPath);
		}

		DWORD dwCount = 0;//已打开进程数
		PROCESSENTRY32 pe32;
		...
		if(hProcessSnap!=INVALID_HANDLE_VALUE)    {
			...
			while(bProcess){
				if(lstrcmp( pe32.szExeFile , szCurPath ) == 0){
					dwCount++;
				}
		...
		if( dwCount > 1 ){
			...
			wsprintf( szName , g_strLoginName+_T("%d%02d%02d(%d).log") ,tmNow->tm_year+1900,tmNow->tm_mon+1,tmNow->tm_mday, dwCount-1);
		}else{
			_tcsftime(szName, 64, g_strLoginName+_T("%Y%m%d.log"), localtime(&now));//用户名作为文件夹名
		}
		_tcscat(_tcscpy(_szFileName, GetFilePath()), szName); 
#endif 
#endif
...
#define LOG_LEVEL(type) CxLog::Instance().SetLevel((type))

#define BASE_LOG(type, msg) CxLog::Instance().Log(__TFILE__, __TFUNCTION__, __LINE__, (type), (msg))
#define LOG(msg) BASE_LOG(CxLog::CX_LOG_INFO, (msg))
#define LOG_DEBUG(msg) BASE_LOG(CxLog::CX_LOG_DEBUG, (msg))
...
#define LOG_BIN(bin, size) BASE_LOG_BIN(CxLog::CX_LOG_INFO, (bin), (size))
...
#define LOGN(msg, ...) BASE_LOGN(CxLog::CX_LOG_INFO, (msg), ##__VA_ARGS__)
#define LOGN_DEBUG(msg, ...) BASE_LOGN(CxLog::CX_LOG_DEBUG, (msg), ##__VA_ARGS__)
...
#define PRE_LOG CxLog::Instance().LogN(__TFILE__, __TFUNCTION__, __LINE__, CxLog::CX_LOG_INFO,
